...
@Component({
  selector: 'app-admin-submissions', standalone: true, imports: [RouterLink, DatePipe, FormsModule],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <div class="page">
      <header class="page-header"><h1>Submissions</h1><p class="desc">Review and manage student submissions.</p></header>
      <div class="filters"><div class="search-wrap"><span class="material-symbols-outlined">search</span><input type="text" [(ngModel)]="search" (ngModelChange)="load()" placeholder="Search by student or assignment..." class="search-input" /></div>
      ...
      <div class="table-wrap"><table class="data-table"><thead><tr><th>Student</th><th>Assignment</th><th>Attempt</th><th>Submitted</th><th>Status</th><th>Score</th></tr></thead>
        <tbody>@for (s of submissions(); track s.id) {
          <tr class="row"><td class="primary">{{ s.studentName }}</td><td>{{ s.assignmentTitle }}</td><td>#{{ s.attempt }}</td><td>{{ s.submittedAt | date:'MMM d, h:mm a' }}</td>
          <td><span class="badge" [attr.data-r]="s.result">{{ fmt(s.result) }}</span></td><td class="mono">{{ s.score }}%</td></tr>
        } @empty { <tr><td colspan="6"><div class="empty"><span class="material-symbols-outlined">inbox</span><p>No submissions found</p></div></td></tr> }</tbody></table></div>
    </div>
  `,
  styles: [`
    ...
    .data-table td{padding:var(--space-3) var(--space-4);font-size:var(--text-sm);color:var(--text-secondary);border-bottom:1px solid var(--border-secondary)}.row{transition:background var(--transition-fast)}.row:hover{background:var(--surface-secondary)}.primary{color:var(--text-primary);font-weight:var(--weight-medium)}.mono{font-family:var(--font-mono)}
...
export class AdminSubmissionsComponent implements OnInit {
  ...
  load(): void {
    this.svc.getSubmissions({ page: 1, pageSize: 20, search: this.search, status: (this.statusFilter || undefined) as EvaluationResult | undefined }).subscribe(r => this.submissions.set(r.data));
  }
